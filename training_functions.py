...
def train(predictor:nn.Module,device:str,dataloader:DataLoader,loss_fn:nn.CrossEntropyLoss,optimizer:torch.optim.AdamW,epoch:int,scheduler,grad_accumulation:int=1):
    loss_tally = 0.0
    ...
    with tqdm(dataloader, desc=f"Epoch {epoch+1}: Training",leave=False) as bar:
        for i, (images, labels) in enumerate(bar):
            ...
            with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                ...
                # Do CE loss as test
                loss = loss_fn(output, labels.squeeze(-1))
